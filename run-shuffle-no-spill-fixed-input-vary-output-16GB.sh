...
# Fixed partition configuration: 768MB partitions, 21 partitions = ~15.75GB total
PARTITION_SIZE_MB=768
NUM_ROWS=50331648
NUM_PARTITIONS=21
...
echo "======================================================================"
echo "Fixed Input Configuration:"
echo "  Partition size: ${PARTITION_SIZE_MB}MB"
echo "  NUM_ROWS per partition: ${NUM_ROWS}"
echo "  NUM_PARTITIONS (input): ${NUM_PARTITIONS}"
echo "  Total size per rank: ${ACTUAL_SIZE_GB}GB"
echo "======================================================================"
echo ""

# --- Sweep over output partition sizes ---
OUTPUT_PARTITION_SIZES=(16 32 64 128 256)

for NUM_OUTPUT_PARTITIONS in "${OUTPUT_PARTITION_SIZES[@]}"; do
    echo "======================================================================"
    echo "Running with ${NUM_OUTPUT_PARTITIONS} output partitions"
    echo "======================================================================"

    # Base APP_ARGS with calculated values
    APP_ARGS="-C ucxx -w 3 -r 1 -g -s -x -b -p ${NUM_PARTITIONS} -o ${NUM_OUTPUT_PARTITIONS} -c ${NUM_COLUMNS} -n ${NUM_ROWS} -x -m async"

    # Output file for this output partition count
    OUTPUT_FILE="no-spill-${PARTITION_SIZE_MB}MB-out${NUM_OUTPUT_PARTITIONS}.txt"
    ...
    sleep 2
    echo "Completed run with ${NUM_OUTPUT_PARTITIONS} output partitions"
    echo ""
done
