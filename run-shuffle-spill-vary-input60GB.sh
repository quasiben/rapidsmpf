...
BYTES_PER_ROW=4
NUM_COLUMNS=4
TOTAL_SIZE_GB=48  # Total input table size per rank in GB
...
# Calculations maintain 48GB total: NUM_ROWS * 4 bytes * 4 columns * NUM_PARTITIONS = 48GB
PARTITION_CONFIGS=(
    "256 16777216 192"     # 256MB partitions
    "512 33554432 96"      # 512MB partitions
    "768 50331648 64"      # 768MB partitions
    "1024 67108864 48"     # 1GB partitions
    "2048 134217728 24"    # 2GB partitions
)
...
for CONFIG in "${PARTITION_CONFIGS[@]}"; do
    ...
    APP_ARGS="-C ucxx -w 3 -r 1 -g -s -x -l $((40 * 1024)) -p ${NUM_PARTITIONS} -o 4 -c ${NUM_COLUMNS} -n ${NUM_ROWS} -x -m async"

    # Output file for this partition size
    OUTPUT_FILE="spill-${PARTITION_SIZE_MB}MB-fixed-input-${TOTAL_SIZE_GB}GB.txt"
